...

{
...

    {
        ...
        public static void UpdateSyncEmailVesselsReportLastSent(int id, DateTime lastSent)

        {
...

            {
                ...
                using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["SISContext"].ConnectionString))

                {

                    conn.Open();

                    using (SqlCommand cmd = new SqlCommand("UPDATE SyncEmailVesselsReport SET LastSent = @LastSent WHERE Id = @Id", conn))

                    {

                        cmd.Parameters.AddWithValue("@LastSent", lastSent);
