...
{
    [Authorize]
    public class DailyNoonController : Controller
    {
        ...
        public ActionResult Index()
        {
           // DailyNoonReport dnR = new DailyNoonReport();
            ...
            //SqlDataAdapter objCMD = new SqlDataAdapter("select a.*,b.FuelType from FuelConsumption a inner join tblFuelType b on a.FuelTypeId=b.Id and a.IsActive=1", ConnectionBulder.con);
        ...
        public ActionResult noonRlist(int? pageNo,string firstVal,string dateF,string dateT)
        {
            ...
            if (string.IsNullOrEmpty(firstVal))
            {
                ...
                dnR.GetNoonRList = CommonMethods.GetNoonReportList("", currPage, pageSize);
                return View(dnR);
            }
            else //if (firstVal != null)
            {
        ...
        public JsonResult GetDisByLeg(int LegId,int CPID)
        {
            using (SqlDataAdapter sda = new SqlDataAdapter("select sum(NoonToNoonDMG_Dist) as totalsum ,sum(stmgtime) as totaltime , (select dtg from VoyageLeg  where id=" + LegId + " and IsActive=1) as dtg,(select cp_log_speed from VoyageLeg  where id=" + LegId + " and IsActive=1) as cpSpeed from dailynoonreport  where LegPortId=" + LegId + " and IsActive=1", ConnectionBulder.con))
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    return Json(new { Result = true, ttlsum = totalSum, ttltime = totalTime, DTG = dtg,CPSpeed= cpsped,Laden=laden,Ballast=ballast,
                    Id5= ViewBag.Id5,
                        Id6 = ViewBag.Id6,
                        Id7 = ViewBag.Id7,
                        Id8 = ViewBag.Id8,
        ...
        public JsonResult Getfuelcons(int NoonReportId)
        {
            ...
            try
            {
                ...
                using (SqlDataAdapter objCMD = new SqlDataAdapter("select value from Fuel_Cons_NR where Noon_Report_Id=" + NoonReportId + " and VesselId=" + vslid + " and ReportType_Id=1  and ConsTypeId not in (1,6)", ConnectionBulder.con))
                {
                    DataTable dt = new DataTable();
                    objCMD.Fill(dt);
                    for (int i = 0; i < dt.Rows.Count; i++)
        ...
        public JsonResult GetNRCargoEdit(int NoonReportId)
        {
            ...
            try
            {
                using (SqlDataAdapter objCMD = new SqlDataAdapter("select a.*,b.cargoname from NR_Cargo a inner join LR_Cargo b on a.lr_cargo_id=b.Id  where  a.VesselId="+ vslid + " and noonreport_id="+NoonReportId+"", ConnectionBulder.con))
                {
                    DataTable dt = new DataTable();
                    objCMD.Fill(dt);
