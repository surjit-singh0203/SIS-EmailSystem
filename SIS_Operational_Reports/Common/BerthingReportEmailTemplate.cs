...
ORDER BY a.Id";
            ...
            return BuildHtmlInline(berthRBind, vesselName, dtNonRoutine, dtMain, dtFuelCons, dtFuelROB, dtBunker, dtCargo);
        }
        ...
        private static string ApplyTemplate(string template, BerthingReport r, string vesselName, DataTable dtNonRoutine, DataTable dtMain, DataTable dtFuelCons, DataTable dtFuelROB, DataTable dtBunker)
        {
            ...
            sb.Append(KvRow("Voy No.", voyNo)).Append(KvRow("Port", r.PortName)).Append(KvRow("Facility Name", facilityName)).Append(KvRow("Berth Name", r.BerthName));
            sb.Append(KvRow("In Port Status", portStatusText)).Append(KvRow("Leg", legText));
            ...
            for (int i = 0; i < nreLabels.Length; i++)
            {
                ...
                // Hrs defaults to "0" when no data — matches webpage which shows 0 instead of "-".
                string hrs = (dtNonRoutine != null && i < dtNonRoutine.Rows.Count) ? (dtNonRoutine.Rows[i]["Hours"]?.ToString() ?? "0") : "0";
            ...
            // Build full fuel consumption table (Daily Noon Report email style — single dtFuelCons arg).
            string fuelConsFullTable = BuildFuelConsFullTable(dtFuelCons);
            sb.Clear();
        ...
        private static string BuildHtmlInline(BerthingReport r, string vesselName, DataTable dtNonRoutine, DataTable dtMain, DataTable dtFuelCons, DataTable dtFuelROB, DataTable dtBunker, DataTable dtCargo)
        {
            string voyNo = r.voyagenumber;
            string legText = "";
            string portStatusText = r.PortStatus.ToString();
            ...
            // Weather → Non-Routine Events → Manoeuvring → Berthing Report Remarks →
            // Engine → Date & Time → LO & HO Consumptions → Other ROB → Fuel ROB in MT →
            // Fuel Consumption in MT (Cargo / Slops ROB / Fresh Water / Ballast follow below).
            ...
            // Non-Routine Events — Event Name header omitted (labels render in column 1);
            // empty Hrs defaults to "0" not "-" per webpage.
            ...
            sb.Append(@"<tr><td style=""padding:6px 8px;border:1px solid #ccc;font-weight:bold;background:#f5f5f5;white-space:nowrap""></td><td style=""padding:6px 8px;border:1px solid #ccc;font-weight:bold;background:#f5f5f5;white-space:nowrap"">Owners/Charterers Account</td><td style=""padding:6px 8px;border:1px solid #ccc;font-weight:bold;background:#f5f5f5;white-space:nowrap;text-align:right"">Hrs.</td></tr>");
            for (int i = 0; i < nreLabels.Length; i++)
            {
                string acc = (dtNonRoutine != null && i < dtNonRoutine.Rows.Count) ? (dtNonRoutine.Rows[i]["ChartererAccount"]?.ToString() ?? "-") : "-";
                string hrs = (dtNonRoutine != null && i < dtNonRoutine.Rows.Count) ? (dtNonRoutine.Rows[i]["Hours"]?.ToString() ?? "0") : "0";
            ...
            // Engine — RPM/BHP forced to 3 decimals per webpage (45 → "45.000", 1209 → "1209.000").
            ...
            sb.Append(@"<tr><td colspan=""8"" style=""padding:0;""><table style=""width:100%;border-collapse:collapse;table-layout:fixed;""><col style=""width:45%;min-width:280px""><col style=""width:55%"">");
            sb.Append(KvRow("SLIP%", r.Slip));
            sb.Append(KvRow("RPM", V3(r.RPM)));
            sb.Append(KvRow("BHP(hp)", V3(r.BHP)));
            sb.Append(KvRow("MCR%", r.MCR));
            sb.Append(@"</table></td></tr>");

            // Date & Time — webpage shows SBE / FWE under a "Date & Time" header column.
            ...
            sb.Append(@"<tr><td colspan=""8"" style=""padding:0;""><table style=""width:100%;border-collapse:collapse;table-layout:fixed;""><col style=""width:45%;min-width:280px""><col style=""width:55%"">");
            sb.Append(@"<tr><td style=""padding:6px 8px;border:1px solid #ccc;font-weight:bold;background:#f5f5f5;""></td><td style=""padding:6px 8px;border:1px solid #ccc;font-weight:bold;background:#f5f5f5;text-align:center;"">Date & Time</td></tr>");
            sb.Append(KvRow("SBE", r.SBE_DateT.HasValue ? r.SBE_DateT.Value.ToString("yyyy-MM-dd HH:mm") : "-"));
            sb.Append(KvRow("FWE", r.RFA_DateT.HasValue ? r.RFA_DateT.Value.ToString("yyyy-MM-dd HH:mm") : "-"));
            sb.Append(@"</table></td></tr>");

            // LO & HO Consumptions — header + row labels match Daily Noon reference; 3-decimal values.
            ...
            sb.Append(@"<tr><td style=""padding:6px 8px;border:1px solid #ccc;font-weight:bold;background:#f5f5f5;"">MECC (Ltrs)</td><td style=""padding:6px 8px;border:1px solid #ccc;text-align:right;"">").Append(V3(r.LO_HO_Cons_MECC)).Append(@"</td><td style=""padding:6px 8px;border:1px solid #ccc;text-align:right;"">").Append(V3(r.LO_HO_Cons_MECC_ROB)).Append(@"</td></tr>");
            sb.Append(@"<tr><td style=""padding:6px 8px;border:1px solid #ccc;font-weight:bold;background:#f5f5f5;"">MECYL (Ltrs)</td><td style=""padding:6px 8px;border:1px solid #ccc;text-align:right;"">").Append(V3(r.LO_HO_Cons_MECYL)).Append(@"</td><td style=""padding:6px 8px;border:1px solid #ccc;text-align:right;"">").Append(V3(r.LO_HO_Cons_MECYL_ROB)).Append(@"</td></tr>");
            sb.Append(@"<tr><td style=""padding:6px 8px;border:1px solid #ccc;font-weight:bold;background:#f5f5f5;"">AECC (Ltrs)</td><td style=""padding:6px 8px;border:1px solid #ccc;text-align:right;"">").Append(V3(r.LO_HO_Cons_AECC)).Append(@"</td><td style=""padding:6px 8px;border:1px solid #ccc;text-align:right;"">").Append(V3(r.LO_HO_Cons_AECC_ROB)).Append(@"</td></tr>");
            sb.Append(@"<tr><td style=""padding:6px 8px;border:1px solid #ccc;font-weight:bold;background:#f5f5f5;"">HYDRAULIC Oil (Ltrs)</td><td style=""padding:6px 8px;border:1px solid #ccc;text-align:right;"">").Append(V3(r.LO_HO_Cons_HYDR_Oil)).Append(@"</td><td style=""padding:6px 8px;border:1px solid #ccc;text-align:right;"">").Append(V3(r.LO_HO_Cons_HYDR_Oil_ROB)).Append(@"</td></tr>");
            sb.Append(@"</table></td></tr>");

            // Other ROB — moved BEFORE Fuel ROB in MT per spec.
            ...
            // Fuel ROB in MT — header rename (no "(SBE/FWE)" suffix) + SBE/FWE as separate columns,
            // trim trailing zeros per webpage (745.200 → "745.2").
            ...
            sb.Append(@"<tr><td colspan=""8"" style=""padding:0;""><table style=""width:100%;border-collapse:collapse;table-layout:fixed;""><col style=""width:40%;min-width:200px""><col style=""width:30%;min-width:150px""><col style=""width:30%;min-width:150px"">");
            sb.Append(@"<tr><td style=""padding:6px 8px;border:1px solid #ccc;font-weight:bold;background:#f5f5f5;""></td><td style=""padding:6px 8px;border:1px solid #ccc;font-weight:bold;background:#f5f5f5;text-align:center;"">SBE</td><td style=""padding:6px 8px;border:1px solid #ccc;font-weight:bold;background:#f5f5f5;text-align:center;"">FWE</td></tr>");
            if (dtFuelROB != null)
            {
                foreach (DataRow dr in dtFuelROB.Rows)
                {
                    string ft = dr["FuelType"]?.ToString() ?? "";
                    string sbe = VTrim(dr.Table.Columns.Contains("SBE") ? dr["SBE"] : null);
                    string fwe = VTrim(dr.Table.Columns.Contains("RFA") ? dr["RFA"] : null);
                    sb.Append(@"<tr><td style=""padding:6px 8px;border:1px solid #ccc;font-weight:bold;background:#f5f5f5;"">").Append(ft).Append(@"</td><td style=""padding:6px 8px;border:1px solid #ccc;text-align:right;"">").Append(sbe).Append(@"</td><td style=""padding:6px 8px;border:1px solid #ccc;text-align:right;"">").Append(fwe).Append(@"</td></tr>");
                }
            }
            sb.Append(@"</table></td></tr>");

            // Fuel Consumption in MT — Daily Noon Report email template copied verbatim
            // (Main Engine / Aux Engine / Boiler / Framo System / IGG & Incinerator / Events / Total).
            ...
            // Cargo — webpage shows "CargoName ( PortName )" as the row label and "Qty(MT)" as the
            // column. One row per BR_Cargo record (joined to LR_Cargo for CargoName + PortName).
            sb.Append(@"<tr><td colspan=""8"" style=""padding:10px 8px;background:#555;color:#fff;font-weight:bold;text-align:center;"">Cargo</td></tr>");
            sb.Append(@"<tr><td colspan=""8"" style=""padding:0;""><table style=""width:100%;border-collapse:collapse;table-layout:fixed;""><col style=""width:45%;min-width:280px""><col style=""width:55%"">");
            sb.Append(@"<tr><td style=""padding:6px 8px;border:1px solid #ccc;font-weight:bold;background:#f5f5f5;""></td><td style=""padding:6px 8px;border:1px solid #ccc;font-weight:bold;background:#f5f5f5;text-align:center;"">Qty(MT)</td></tr>");
            ...
            // Slops ROB — row label "ROB (m3)" → "ROB"; 3-decimal values per webpage.
            ...
            // Ballast — row label "ROB (MT)" → "ROB"; 3-decimal value.
        ...
        private static string FuelConsSection(string sectionName, (string fuelType, decimal sea, decimal man, decimal wait, decimal berth, decimal subTotal)[] rows, bool showSubTotal = true)
        {
            ...
            s.Append("<tr><td style=\"").Append(hdr).Append("\">Fuel</td>");
        ...
        private static string FuelConsEventsSectionFromModel(BerthingReport r)
        {
            ...
            s.Append("<tr>");
            s.Append("<td style=\"").Append(hdr).Append("\">Fuel</td>");
            s.Append("<td style=\"").Append(hdr).Append("\">Stoppage</td>");
            s.Append("<td style=\"").Append(hdr).Append("\">Deviation</td>");
            s.Append("<td style=\"").Append(hdr).Append("\">Slow Stm.</td>");
            s.Append("<td style=\"").Append(hdr).Append("\">Bad Wthr.</td>");
            s.Append("<td style=\"").Append(hdr).Append("\">COT Prep</td>");
            s.Append("<td style=\"").Append(hdr).Append("\">Cargo Htg.</td>");
            s.Append("<td style=\"").Append(hdr).Append("\">BW Exch.</td>");
        ...
        // ── Daily Noon Report style Fuel Consumption helpers — copied verbatim from
        // DailyNoonReportEmailTemplate with two customisations per Berthing webpage:
        //  • column header "Manoeuv." → "MANOEUV" (all-caps to match webpage)
        //  • Events column label "Stoppage" → "Stoppage at Sea" (long form)
        // All other CSS / structure / loop pattern / decimal formatting are identical.
        // FcFmt forces always-3-decimal output (e.g. 0 → "0.000", 0.8 → "0.800").
        ...
        private static string FuelConsEventsTable(DataTable dtFuelCons)
        {
            ...
            s.Append("<tr>")
              .Append("<th style=\"").Append(FC_TH).Append("\">Fuel</th>");
            foreach (string lbl in eventLabels)
                s.Append("<th style=\"").Append(FC_TH).Append("\">").Append(lbl).Append("</th>");
            s.Append("</tr>");
        ...
        /// <summary>Daily Noon-style Fuel Consumption table — driven entirely by dtFuelCons (ConsTypeId mapping).
        /// 1-5=ME, 6-10=AE (1 and 6 are CP, filtered out in fetch), 11-14=BLR, 15-18=FRAMO,
        /// 19=IGG, 20-27=Events, 28=Incinerator.</summary>
