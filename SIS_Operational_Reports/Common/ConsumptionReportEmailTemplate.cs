...
{
    /// <summary>
    /// Builds HTML email body for Consumption Report. Uses ConsumptionReport.html template when available.
    /// Uses DailyNoonReport data model, structured with Navigation / Engine / Cargo sections focused on consumption data.
    /// </summary>
    public static class ConsumptionReportEmailTemplate
    {
        private const string DateFormat = "dd-MMM-yyyy";
        private const string DateTimeFormat = "dd-MMM-yyyy HH:mm";
        private const string TemplatePath = "~/Templates/ConsumptionReport.html";
        ...
        private static string ApplyTemplate(string template, DailyNoonReport r, string vesselName, DataTable dtNonRoutine, DataTable dtMain, DataTable dtFuelCons, DataTable dtFuelROB, DataTable dtBunker)
        {
            ...
            sb.Append(KvRow("Voy No.", voyNo)).Append(KvRow("Status", r.VesselStatus)).Append(KvRow("Laden/Ballast", r.VesselStatus));
            sb.Append(KvRow("At Sea/In Port", r.AtSeaOrPort)).Append(KvRow("In Port Status", portStatusText));
            sb.Append(KvRow("Leg", legText)).Append(KvRow("Report Date", V(r.Date))).Append(KvRow("Draft Mid (Mtrs)", r.DraftMid));
            string headerRows = sb.ToString();
            sb.Clear();

            // --- Speed - Distance ---
            sb.Append(KvRow("Dist Noon to Noon (DMG)(NM)", r.NoonToNoonDMG_Dist));
            sb.Append(KvRow("Actual Speed Noon to Noon(Kts)", r.Act_Speed));
            sb.Append(KvRow("SLIP%", r.Slip)).Append(KvRow("RPM", r.RPM));
        ...
        private static string BuildHtmlInline(DailyNoonReport r, string vesselName, DataTable dtNonRoutine, DataTable dtMain, DataTable dtFuelCons, DataTable dtFuelROB, DataTable dtBunker)
        {
            ...
            sb.Append(@"<p style=""margin:0 0 12px 0;"">The Consumption Report for vessel ").Append(System.Web.HttpUtility.HtmlEncode(vesselName ?? "")).Append(@" dated ").Append(V(r.Date)).Append(@" has been successfully generated and is attached to this email in Excel format for your reference.</p>");
            sb.Append(@"<table style=""width:100%;min-width:1200px;max-width:1200px;border-collapse:collapse;border:1px solid #ccc;"">");

            // Navigation header
            sb.Append(@"<tr><td colspan=""8"" style=""padding:12px;background:#555;color:#fff;font-size:16px;font-weight:bold;text-align:center;"">Consumption Report - Navigation (").Append(V(vesselName)).Append(@")</td></tr>");
            ...
            // Speed - Distance
            sb.Append(@"<tr><td colspan=""8"" style=""padding:10px 8px;background:#555;color:#fff;font-weight:bold;text-align:center;"">Speed - Distance</td></tr>");
            sb.Append(@"<tr><td colspan=""8"" style=""padding:0;""><table style=""width:100%;border-collapse:collapse;table-layout:fixed;""><col style=""width:45%;min-width:280px""><col style=""width:55%"">");
            sb.Append(KvRow("Dist Noon to Noon (DMG)(NM)", r.NoonToNoonDMG_Dist));
            sb.Append(KvRow("Actual Speed Noon to Noon(Kts)", r.Act_Speed));
            sb.Append(KvRow("SLIP%", r.Slip)).Append(KvRow("RPM", r.RPM));
            ...
            // Remarks
            sb.Append(@"<tr><td colspan=""8"" style=""padding:10px 8px;background:#555;color:#fff;font-weight:bold;text-align:center;"">Consumption Report Remarks</td></tr>");
