    ...
    public class PumpController : BaseController
    {
        ...
        public ActionResult addpump(PumpClass cls)
        {
            if (cls.Id == 0)
            {
                ...
                int checkExistense = CommonMethods.CheckpumpExistence(cls, cls.VesselId, 0, 0, "Insert");
                if (checkExistense == 0)
                {
                    cls.PumpUseId = 1;
            ...
            {
                cls.VesselId = Convert.ToInt32(Session["ImoNo"]);
                int checkExistense = CommonMethods.CheckpumpExistence(cls, cls.VesselId, 0, cls.Id, "Update");
                if (checkExistense == 0)
                {
        ...
        public ActionResult InsertInOrder(string pumplist)
        {
            ...
            {                
                ...
                {
                    count++;
                    CommonMethods.InsertinOrder(count, rootObject.Name, 0, "Pump");
                }
            }
            catch { }

            TempData["Success"] = "Record successfully saved !";
            return Json(Url.Action("Index", "Pump"));
