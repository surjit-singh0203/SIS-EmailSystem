...
namespace SIS_Operational_Reports.Areas.Report.Controllers
{
    ...
    public class BunkerController : BaseController
    {
        // GET: Report/Bunker
        ...
        public ActionResult _BunkerReport(int? pageNo, string firstVal, string Vessel)
        {
            BunkerReport BR = new BunkerReport();
            // BR.VesselList = CommonClass.GetVesselList(StaticHelper.PermittedVessel);
        ...
        public ActionResult InsertBunkerReport(BunkerReport model)
        {
            ...
            try
            {
              //  model.VesselId = Convert.ToInt32(Session["VesselID"]);
                if (model.Id != 0)
                {
                    model.BunkerReportList = CommonMethods.editBunkerRList(model.Id, model.VesselId, "BunkerReport");
                    ...
                    if (model.LabAnalysisReport != null && model.LabAnalysisReport.ContentLength > 0)
                    {
                        ...
                        {
                            string path = Server.MapPath("~/Bunker_LabAnalysisReport/" + BunkerRBind.LabAnalysisReport_Name);
                            ...
                            string filePath = Path.Combine(Server.MapPath("~/Bunker_LabAnalysisReport"), fileName);
        ...
        {
            BunkerReport BR = new BunkerReport();
            BR.BunkerReportList = CommonMethods.editBunkerRList(id, vslid ,"BunkerReport");
            var BunkerRBind = BR.BunkerReportList.Where(x => x.Id == id).FirstOrDefault(e => e.Id == id);
            var path = Path.Combine(Server.MapPath("~/Bunker_LabAnalysisReport"), BunkerRBind.LabAnalysisReport_Name);
            ...
            ViewBag.GetPortEdit = string.Format("BindPrtEdit('{0}');", BunkerRBind.PortName);
        ...
        public ActionResult InsertBunkerFuelType(string BunkerFuelTypeList)
        {
            ...
            foreach (var rootObject in Json)
            {
                ...
                if (BR_ID == "")
                {
                    using (SqlDataAdapter adp = new SqlDataAdapter("select MAX(Id) from BunkerReport where VesselId=" + rootObject.VesselId + "", ConnectionBulder.con))
                    {
                ...
                {
                    rootObject.MaxR_Id = Convert.ToInt32(Session["BR_ID"]);
                }
        ...
        public ActionResult OpenPDF(string fileName, string fileExtension)
        {
            ...
            string path = Server.MapPath("~/Bunker_LabAnalysisReport/" + fileName + fileExtension);
