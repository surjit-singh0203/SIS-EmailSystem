...
namespace SIS_Operational_Reports.Areas.Report.Controllers
{
    ...
    public class LoadingController : BaseController
    {
        ...
        public ActionResult insertLodingR(LoadingReport _lodingR)
        {
            ...
            //int vslid = Convert.ToInt32(Session["VesselID"]);
            int vslid = Convert.ToInt32(Session["EditVesselIDLoading"]);
            ...
            if (Convert.ToInt32(Session["LR_ID"]) != 0)
            {

                _lodingR.Id = Convert.ToInt32(Session["LR_ID"]);
        ...
        public JsonResult BindPort_Edit(int Voyid)
        {
            ...
            try
            {
                ...
                using (SqlDataAdapter sda = new SqlDataAdapter("select PortName  from LoadingReport where id=" + id + " and VesselId="+ vslid + "", ConnectionBulder.con))
                {
                    DataTable tbl = new DataTable();
                    sda.Fill(tbl);
                    if (tbl.Rows.Count > 0)
        ...
        public ActionResult InsertCargoList(string CargoListing)
        {
           ...
           // int vslid = Convert.ToInt32(Session["VesselID"]);
            ...
            int lodRId = Convert.ToInt32(Session["EditId"]);

            var loadRep = CommonMethods.GetSingleLoadingReport(lodRId, vslid, "LoadingReport");
        ...
        public ActionResult InsertBllastPumpUse(string ballasttank)
        {
            ...
            //int id= Convert.ToInt32(Session["EditId"]);
            // vd.LR_Ballast_PumpUseList = GetPumpsINUse(id, vslid);
            ...
            foreach (var rootObject in Json)
            {
                ...
                if (Session["LR_ID"].ToString() != "")
                {
                    ...
                    CommonMethods.InsertUpdateLoadingBallast_PumpUse(rootObject, "Update_LR");
                }
            }
            return View();
        }

        public ActionResult LoadingRList(int? pageNo, string firstVal, string dateF, string dateT, string Vessel)
        {
            //int vslid = Convert.ToInt32(Session["VesselID"]);
            ...
            Loading.VesselList = CommonClass.GetVesselList(StaticHelper.PermittedVessel);
            ...
            if (firstVal == null)
            {
                ...
                Loading.VesselId = Convert.ToInt32(Vessel);
                return View(Loading);
            }
        ...
        {
            LoadingReport vd = new LoadingReport();
            vd.LoadingReportList = CommonMethods.editloadingRList(id, vesselid, "LoadingReport");
           ...
           // Session["VesselID"] = vesselid;
            Session["EditVesselIDLoading"] = vesselid;


            Session["LR_ID"] = loadingRBind.Id;
        ...
        public ActionResult DownloadExcel(int id, int vesselid)
        {
            ...
            DataSet ds = CommonMethods.ExportReportList(id, vesselid, "LoadingReport");
            using (wb = new XLWorkbook())
            {
                foreach (DataTable table in ds.Tables)
                {
                    using (SqlDataAdapter adp = new SqlDataAdapter("select a.Rate,b.Name from LR_DCR_PumpsUse a left join tblPump b on a.PumpId=b.Id where a.LRId=" + id + " and a.VesselId =" + vesselid + "", ConnectionBulder.con))
                    {
                        DataTable dt = new DataTable();
                        adp.Fill(dt);
                        int chk = 0;
                    ...
                    /////   for stoppage reason
                    ///
                    using (SqlDataAdapter adp = new SqlDataAdapter("select Stoppage,DateTimeFrom,DateTimeTo from LR_Stoppage where LRId=" + id + " and VesselId =" + vesselid + "", ConnectionBulder.con))
                    {
                        DataTable dt = new DataTable();
                        adp.Fill(dt);
                        int chk = 0;
                ...
                Response.AddHeader("content-disposition", "attachment;filename=LoadingReport_Sis_Nova_" + DateTime.Now.ToString("ddMMyyyy") + "_.xlsx");
        ...
        public ActionResult EditFromDashboard(string reportdate, int vesselid, string Exactdate = "")
        {
            ...
            if (Exactdate != "")
            {
                vd.LoadingReportList = CommonMethods.editloadingRListDashboard(reportdate, vesselid, "LoadingReportR");
            }
            if (Exactdate == "")
            {
                vd.LoadingReportList = CommonMethods.editloadingRListDashboard(reportdate, vesselid, "LoadingReport");
            }
            var ID = vd.LoadingReportList.Select(x => x.Id).FirstOrDefault();
            var loadingRBind = vd.LoadingReportList.Where(x => x.Id == ID).FirstOrDefault(e => e.Id == ID);
            //ViewBag.JavaScriptFunction = string.Format("Getfuelcons11('{0}');", noonRBind.VoyageId);
            TempData["loadingReportId"] = ID;
            Session["EditId"] = ID;
