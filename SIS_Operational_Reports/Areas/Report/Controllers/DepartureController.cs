    ...
    public class DepartureController : BaseController
    {
            ...
            try
            {
                ...
                foreach (var rootObject in Json)
                {
                    ...
                    if (k == 0)
                    {
                        if (Session["DR_ID"].ToString() != "")
                        {
                            using (SqlDataAdapter adp1 = new SqlDataAdapter("delete from tbl_BunkerLReceipt where TableMax_Id=" + DRID + " and VesselId=" + vslid + " and ReportType_Id=3", ConnectionBulder.con))
                            {
        ...
        public JsonResult Getfuelcons(int NoonReportId)
        {
            ...
            try
            {
                ...
                using (SqlDataAdapter objCMD = new SqlDataAdapter("select value, ConsTypeId from Fuel_Cons_NR where Noon_Report_Id=" + NoonReportId + " and VesselId=" + vslid + " and ReportType_Id=3  and ConsTypeId not in (1,6)", ConnectionBulder.con))
                {
                    DataTable dt = new DataTable();
                    objCMD.Fill(dt);
                    int k = 0;
        ...
        public ActionResult Edit(int id)
        {
            ...
            vd.GetDepRList = CommonMethods.editdepartureRList(id, "DepartureReport");
        ...
        public JsonResult BindPort_Edit(int Voyid)
        {
            ...
            try
            {
                ...
                using (SqlDataAdapter sda = new SqlDataAdapter("select DeparturePort  from DepartureReport where id=" + id + "", ConnectionBulder.con))
                {
                    DataTable tbl = new DataTable();
                    sda.Fill(tbl);
                    if (tbl.Rows.Count > 0)
        ...
        public JsonResult BindPortEdit1(int Legid)
        {
            ...
            try
            {
                ...
                using (SqlDataAdapter sda = new SqlDataAdapter("select NextPort  from DepartureReport where id=" + id + "", ConnectionBulder.con))
                {
                    DataTable tbl = new DataTable();
                    sda.Fill(tbl);
                    if (tbl.Rows.Count > 0)
        ...
        public JsonResult GetDRCargoEdit(int DepReportId)
        {
            ...
            try
            {
                using (SqlDataAdapter objCMD = new SqlDataAdapter("select a.*,CONVERT(VARCHAR(20),a.completion_DateT,120)  completionDate,b.cargoname, b.PortName from DR_Cargo a inner join LR_Cargo b on a.lr_cargo_id=b.Id  where  a.VesselId=" + vslid + " and depreport_id=" + DepReportId + "", ConnectionBulder.con))
                {
                    DataTable dt = new DataTable();
                    objCMD.Fill(dt);
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        drCargoName.Add(dt.Rows[i]["completionDate"]);
        ...
        public ActionResult InsertDRCargo(string drcargo)
        {
            ...
            foreach (var rootObject in Json)
            {
                ...
                if (Session["DR_ID"].ToString() != "")
                {
                    ...
                    if (k == 0)
                    {
                        using (SqlDataAdapter adp = new SqlDataAdapter("delete from DR_Cargo where DepReport_Id=" + noonReportId + " and VesselId=" + vslid + "", ConnectionBulder.con))
                        {
                            DataTable dt = new DataTable();
                            adp.Fill(dt);
                            k = 1;
                    ...
                    CommonMethods.InsertUpdateDRCargo(noonReportId, cls, "Insert", "DepartureReport");
        ...
        //    {
        //        var ws = wb.Worksheets.Add(dt, "DepartureReport");
        ...
        //            string fileName = "DepartureReport" + DateTime.Now.ToString("ddMMyyyyHHmmss") + ".xlsx";
        ...
        public ActionResult SyncReportDownload(int VoyageId, int id, string ReportDate)
        {
            ...
            try
            {
                ...
                vd.GetDepRList = CommonMethods.editdepartureRList(id, "DepartureReport");
                depRBind = vd.GetDepRList != null ? vd.GetDepRList.Where(x => x.Id == id).FirstOrDefault(e => e.Id == id) : null;

                if (depRBind != null)
                {
                    try
                    {
                        using (SqlDataAdapter adp = new SqlDataAdapter("SELECT b.FuelType, a.EOSP AS SBE, a.FWE AS RFA FROM tbl_FuelROB a INNER JOIN tblFuelType b ON a.FuelType_Id=b.Id WHERE a.TableMax_Id=" + id + " AND a.VesselId=" + vslid + " AND a.ReportType_Id=3 ORDER BY a.FuelType_Id", ConnectionBulder.con))
                        {
                            adp.Fill(dtFuelROB);
                        }
                    }
                    ...
                    try
                    {
                        using (SqlDataAdapter adp = new SqlDataAdapter("SELECT ChartererAccount, Hours FROM tblNonRoutineCommon WHERE Report_Table_Id=3 AND ReportType_Id=" + id + " AND VesselId=" + vslid + " AND IsActive=1 ORDER BY Id", ConnectionBulder.con))
                        {
                    ...
                    try
                    {
                        using (SqlDataAdapter adp = new SqlDataAdapter("SELECT a.*, b.CargoName, b.PortName FROM DR_Cargo a INNER JOIN LR_Cargo b ON a.LR_Cargo_Id=b.Id AND a.VesselId=b.VesselId WHERE a.VesselId=" + vslid + " AND a.DepReport_Id=" + id, ConnectionBulder.con))
                        {
                            adp.Fill(dtDRCargo);
                        }
                    }
                    catch
                    {
                        try
                        {
                            using (SqlDataAdapter adp = new SqlDataAdapter("SELECT a.*, b.CargoName, b.PortName FROM DR_Cargo a INNER JOIN LR_Cargo b ON a.lr_cargo_id=b.Id WHERE a.VesselId=" + vslid + " AND a.DepReport_Id=" + id, ConnectionBulder.con))
                            {
                                adp.Fill(dtDRCargo);
                            }
                        }
            ...
            using (XLWorkbook wb = new XLWorkbook())
            {
                AddDepartureNavigationSheet(wb, depRBind, dtNonRoutine, dt);
                AddDepartureEngineSheet(wb, depRBind, dtFuelROB, dtFuelCons);
        ...
        private void AddDepartureNavigationSheet(XLWorkbook wb, DepartureReport r, DataTable dtNonRoutine, DataTable dtMain)
        {
            ...
            if (r != null)
            {
                ...
                AddKeyValueSingle(ws, ref row, "Next Port", r.NextPort ?? "");
                AddKeyValueSingle(ws, ref row, "Dep. Port", r.DeparturePort ?? "");
                AddKeyValueSingle(ws, ref row, "Report Date", r.ReportDate != null ? Convert.ToDateTime(r.ReportDate).ToString(CommonMethods.ExcelDateFormat) : "");
                AddKeyValueSingle(ws, ref row, "ETA", r.ETA);
            ...
            if (r != null)
            {
                AddKeyValueSingle(ws, ref row, "Manoeuvring Hours", r.Manoeuvring_Hrs ?? 00);
            ...
            if (r != null)
            {
                AddKeyValueSingle(ws, ref row, "CTM (Mention Currency & Amount)", r.CTM ?? "");
                AddKeyValueSingle(ws, ref row, "SPARES (Mention Revision Numbers)", r.Spares ?? "");
                AddKeyValueSingle(ws, ref row, "STORES (Mention Revision Numbers)", r.Stores ?? "");
                AddKeyValueSingle(ws, ref row, "REPAIRS CONDUCTED (Mention details)", r.RepairsConducted ?? "");
                AddKeyValueSingle(ws, ref row, "CREW CHANGE (No of Crew)", r.CrewChange ?? 00);
                AddKeyValueSingle(ws, ref row, "ITEMS LANDED (Mention Details)", r.ItemsLanded ?? "");
            }
            row++;
        ...
        private void AddDepartureEngineSheet(XLWorkbook wb, DepartureReport r, DataTable dtFuelROB, DataTable dtFuelCons)
        {
            ...
            if (r != null)
            {
                ws.Cell(row, 1).Value = "Slops Disposed(m3)";
                ws.Cell(row, 1).Style.Font.Bold = true;
                ws.Cell(row, 2).Value = SafeValue(r.SlopsDisposed_Oil, true);
                ws.Cell(row, 3).Value = SafeValue(r.SlopsDisposed_Water, true);
                ws.Cell(row, 4).Value = SafeValue(r.SlopsDisposed_Total, true);
                row++;
                ws.Cell(row, 1).Value = "Slops ROB(m3)";
                ws.Cell(row, 1).Style.Font.Bold = true;
                ws.Cell(row, 2).Value = SafeValue(r.SlopsROB_Oil, true);
                ws.Cell(row, 3).Value = SafeValue(r.SlopsROB_Water, true);
                ws.Cell(row, 4).Value = SafeValue(r.SlopsROB_Total, true);
                row++;
                ws.Cell(row, 1).Value = "Bilge Disposed(m3)";
                ws.Cell(row, 1).Style.Font.Bold = true;
                ws.Cell(row, 2).Value = SafeValue(r.BilgesDisposed_Oil, true);
                ws.Cell(row, 3).Value = SafeValue(r.BilgesDisposed_Water, true);
                ws.Cell(row, 4).Value = SafeValue(r.BilgesDisposed_Total, true);
                row++;
                ws.Cell(row, 1).Value = "Bilge ROB(m3)";
                ws.Cell(row, 1).Style.Font.Bold = true;
                ws.Cell(row, 2).Value = SafeValue(r.BilgesROB_Oil, true);
                ws.Cell(row, 3).Value = SafeValue(r.BilgesROB_Water, true);
                ws.Cell(row, 4).Value = SafeValue(r.BilgesROB_Total, true);
                row++;
            }
            row++;

            ws.Cell(row, 1).Value = "Other Disposal";
            var rngOtherDisp = ws.Range(row, 1, row, 3);
            rngOtherDisp.Merge();
            rngOtherDisp.Style.Font.Bold = true;
            rngOtherDisp.Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Center;
            ApplyGridTitleStyle(rngOtherDisp);
            row++;
            if (r != null)
            {
                AddKeyValueSingle(ws, ref row, "Sludge(m3)", r.Sludge, true);
                AddKeyValueSingle(ws, ref row, "Garbage - Plastic(m3)", r.GarbagePlastic, true);
                AddKeyValueSingle(ws, ref row, "Garbage - Others(m3)", r.GarbageOthers, true);
                AddKeyValueSingle(ws, ref row, "Other Disposal(m3)", r.OtherDisposal, true);
            }
            row++;

            ws.Cell(row, 1).Value = "Date & Time";
            var rngDT = ws.Range(row, 1, row, 3);
            rngDT.Merge();
            ...
            if (r != null)
            {
                AddKeyValueSingle(ws, ref row, "SBE", r.SBE_DateT);
                AddKeyValueSingle(ws, ref row, "RFA", r.RFA_DateT);
            }
            row++;
            ...
            if (dtFuelROB != null && dtFuelROB.Rows.Count > 0)
            {
                foreach (DataRow dr in dtFuelROB.Rows)
                {
                    ...
                    ws.Cell(row, 2).Value = SafeValue(dr.Table.Columns.Contains("SBE") ? dr["SBE"] : (dr.Table.Columns.Contains("EOSP") ? dr["EOSP"] : null), true);
                    ws.Cell(row, 3).Value = SafeValue(dr.Table.Columns.Contains("RFA") ? dr["RFA"] : (dr.Table.Columns.Contains("FWE") ? dr["FWE"] : null), true);
                    row++;
                }
            }
            else if (r != null)
            {
                ...
                ws.Cell(row, 2).Value = SafeValue(r.SBE_ROB, true);
                ws.Cell(row, 3).Value = SafeValue(r.RFA_ROB, true);
                row++;
            ...
            if (r != null)
            {
                ...
                ws.Cell(row, 3).Value = SafeValue(r.Bunker_LO_Rec_MECC, true);
                ws.Cell(row, 4).Value = SafeValue(r.LO_HO_Cons_MECC_ROB, true);
                row++;
                ws.Cell(row, 1).Value = "MECYL(Ltrs)";
                ws.Cell(row, 2).Value = SafeValue(r.LO_HO_Cons_MECYL, true);
                ws.Cell(row, 3).Value = SafeValue(r.Bunker_LO_Rec_MECYL, true);
                ws.Cell(row, 4).Value = SafeValue(r.LO_HO_Cons_MECYL_ROB, true);
                row++;
                ws.Cell(row, 1).Value = "AECC(Ltrs)";
                ws.Cell(row, 2).Value = SafeValue(r.LO_HO_Cons_AECC, true);
                ws.Cell(row, 3).Value = SafeValue(r.Bunker_LO_Rec_AECC, true);
                ws.Cell(row, 4).Value = SafeValue(r.LO_HO_Cons_AECC_ROB, true);
                row++;
                ws.Cell(row, 1).Value = "HYDRAULIC Oil(Ltrs)";
                ws.Cell(row, 2).Value = SafeValue(r.LO_HO_Cons_HYDR_Oil, true);
                ws.Cell(row, 3).Value = SafeValue(r.Bunker_LO_Rec_HYDR_Oil, true);
                ws.Cell(row, 4).Value = SafeValue(r.LO_HO_Cons_HYDR_Oil_ROB, true);
                row++;
            }
            row++;
            ...
            ws.Range(row, 1, row, 30).Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Center;
            row++;
            WriteDepartureFuelConsRow(ws, ref row, "VLSFO", dtFuelCons);
            WriteDepartureFuelConsRow(ws, ref row, "MDO", dtFuelCons);
        ...
        private void WriteDepartureFuelConsRow(IXLWorksheet ws, ref int row, string fuelType, DataTable dtFuelCons)
        {
            var vals = GetDepartureFuelConsByFuelType(dtFuelCons, fuelType);
        ...
        private static decimal[] GetDepartureFuelConsByFuelType(DataTable dt, string fuelType)
        {
        ...
        private void AddDepartureCargoSheet(XLWorkbook wb, DepartureReport r, DataTable dtDRCargo)
        {
            ...
            if (dtDRCargo != null && dtDRCargo.Rows.Count > 0)
            {
                foreach (DataRow dr in dtDRCargo.Rows)
                {
                    ...
                    ws.Cell(row, 4).Value = FormatCargoDecimal(dr.Table.Columns.Contains("Cargo_Temp") ? dr["Cargo_Temp"] : null);
                    ws.Cell(row, 5).Value = dr.Table.Columns.Contains("Completion_DateT") ? SafeValue(dr["Completion_DateT"]) : "";
                    ws.Cell(row, 6).Value = FormatCargoDecimal(dr.Table.Columns.Contains("Rate") ? dr["Rate"] : null);
                    row++;
                }
            }
            row++;
            ...
            row++;
            AddKeyValueSingle(ws, ref row, "ROB (MT)", r?.Ballast_ROB ?? 00, true);
            row++;
        ...
        private static string FormatCargoDecimal(object val)
        {
            ...
            decimal d;
            if (decimal.TryParse(val.ToString(), out d)) return d.ToString("0.000");
            return val.ToString();
        }
    }
}