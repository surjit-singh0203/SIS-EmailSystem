...
namespace SIS_Operational_Reports.Areas.CharterParty.Controllers
{
    ...
    public class CPVoyageController : BaseController
    {
        // GET: CharterParty/CPVoyage
        public static int NewMaxCPid { get; set; }
        public ActionResult Index(int? pageNo, string firstVal, string dateF, string dateT)
        {
            string VesselId = Session["VesselID"].ToString();
            int vslid = Convert.ToInt32(VesselId);
            CharterPartyClass cpm = new CharterPartyClass();
            //cpm.CPVesselInfo = CommonMethods.GetCharterPartyList(VesselId);
            ...
            if (firstVal == null)
            {
                cpm.CPVesselInfo = CommonMethods.GetCharterPartyList(vslid, currPage, pageSize);
                return View(cpm);
            }
            ...
            {
                cpm.CPVesselInfo = CommonMethods.SearchCPVoyageDList(vslid, currPage, pageSize, firstVal, dateF, dateT, "CPVoyageDetails");
                return PartialView("_searchCPVoyage", cpm);
            }
